...
Screen_Module3_SettingsView::Screen_Module3_SettingsView()
{
	...
	for (int i = 0; i < SETTABLE_PARAMETER_NAME_COUNT; i++)
	{
		...
		{
			settableParameterStringDisplay[i][character] = Model::m_InitParametersModule3[i + SETTABLE_PARAMETER_NAME_OFFSET][character];
		}
	}
...
void Screen_Module3_SettingsView::getScrollWheelValues(int values[])
{
...
void Screen_Module3_SettingsView::translateScrollWheelValues(int values[])
{
	for (int i = 0; i < PAYLOAD_SIZE; i++)
	{
		switch (values[i])
...
double Screen_Module3_SettingsView::processScrollWheelValues(const int values[])
{
