...
Screen_Module2_SettingsView::Screen_Module2_SettingsView()
{
	...
	for (int i = 0; i < SETTABLE_PARAMETER_NAME_COUNT; i++)
	{
		...
		{
			settableParameterStringDisplay[i][character] = Model::m_InitParametersModule2[i + SETTABLE_PARAMETER_NAME_OFFSET][character];
		}
	}
...
void Screen_Module2_SettingsView::getScrollWheelValues(int values[])
{
...
void Screen_Module2_SettingsView::translateScrollWheelValues(int values[])
{
	for (int i = 0; i < PAYLOAD_SIZE; i++)
	{
		switch (values[i])
...
double Screen_Module2_SettingsView::processScrollWheelValues(const int values[])
{
